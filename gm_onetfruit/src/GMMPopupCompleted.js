let GMMPopupCompleted = GMMPopupAbstract.extend({
    lstStar: [],
    txtMoney: null,
    txtTime: null,
    btnNext: null,
    ...
    initUI: function () {
        let pnlStar = this._mainPanel.getChildByName("pnlStar");
        for (let i = 1; i <= 3; i++) {
            let star = pnlStar.getChildByName("star" + i);
    ...
    onTouchButton: function(pRef, touchType) {
        if (GMMGameUtils.makeEffectButton(pRef, touchType, 1.0, 0.9)) {
            ...
            this.onCloseCallback = () => {
                if (pRef === this.btnMenu) {
                    if (this.onHomeCallback) {
                        this.onHomeCallback();
                    }
                } else if (pRef === this.btnNext) {
    ...
    reloadUI: function(nLevel, nTime, nStar, nBonus) {
        ...
        for (let i =0; i< this.lstStar.length;i++) {
            let star = this.lstStar[i];
            star.setScale(0);
        }
        let scaleIn = cc.EaseBackOut.create(cc.scaleTo(0.7, 1.0));
        switch (nStar) {
            case 1: {
        ...
        /// save data
        let numHint = getPrefInteger(PREF_LOG_NUM_HINT);
        let numTime = getPrefInteger(PREF_LOG_NUM_TIME);
