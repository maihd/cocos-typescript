var _instance;
var NUMBER_PADDING_LINE = 8;
var NUMBER_DELAY_ANIM_EX = 0.4;
var COLOR_DRAW_LINE = cc.color(255, 255, 255);
var GMMMatrixOnet = cc.Node.extend({
    ...
    reload: function (obj) {
        ...
        this.typeChangeMatrix = this.gamePlay.currLevel % type_change.length;
        var index = 0;
        this.currEat = 0;
    ...
    changeMatrix: function (i1, j1, i2, j2) {
        switch (this.typeChangeMatrix) {
            ...
            case type_change.Move_to_Top:
                ...
                break;
            case type_change.Move_ZIG_ZAG_VER:
                this.moveZigZagVer(i1, j1, i2, j2);
                break;
            case type_change.Move_ZIG_ZAG_HOL:
    ...
    moveBot: function (i1, j1, i2, j2) {
        ...
        for (i = i2; i > 0; i--) {
            ...
            this.matrix[iA] = this.matrix[iB];
            var itemB_1 = this.getChildOnet(iB);
            itemB_1.stopAllActions();
            this.actionMoveTo(itemB_1, cc.p(this.xStart + j2 * this.sizeItem, this.yStart - i * this.sizeItem));
            itemB_1.setName("ITEM" + iA);
        }
        iA = i * this.nCol + j2;
        this.matrix[iA] = -1;
    },
    moveZigZagVer: function (i1, j1, i2, j2) {
        ...
        if (inRow) {
            for (i = j2; i > 0; i--) {
                ...
                this.matrix[iA] = this.matrix[iB];
                var itemB_2 = this.getChildOnet(iB);
                itemB_2.stopAllActions();
                this.actionMoveTo(itemB_2, cc.p(this.xStart + i * this.sizeItem, this.yStart - i2 * this.sizeItem));
                itemB_2.setName("ITEM" + iA);
            }
        }
        else {
            for (i = j2; i < this.nCol - 1; i++) {
                ...
                this.matrix[iA] = this.matrix[iB];
                var itemB_3 = this.getChildOnet(iB);
                itemB_3.stopAllActions();
                this.actionMoveTo(itemB_3, cc.p(this.xStart + i * this.sizeItem, this.yStart - i2 * this.sizeItem));
                itemB_3.setName("ITEM" + iA);
            }
        }
        iA = i2 * this.nCol + i;
        this.matrix[iA] = -1;
    ...
    actionMoveTo: function (node, pos) {
        var move = cc.EaseBackOut.create(cc.moveTo(NUMBER_DELAY_ANIM_EX * 2, pos));
        node.runAction(move);
    },
    ...
    drawLines: function (x1, y1, x2, y2, line_point) {
        ...
        var call = cc.CallFunc.create(function () {
            ...
            if (_instance.currEat === _instance.matrix.length) {
                ...
                if (_this.onCompletedCallback) {
                    _this.onCompletedCallback();
                }
            }
    ...
    checkChooseItemOnet: function (currPoint) {
        for (var i = 0; i < _instance.matrix.length; i++) {
            if (_instance.matrix[i] != -1 && _instance.isCollisionRect(_instance.getChildOnet(i).getPosition(), currPoint, _instance.sizeItem / 2)) {
                ...
                else if (_instance.currIndexFirst != i) {
